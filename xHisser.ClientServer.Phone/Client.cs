...
namespace Hisser.ClientServer
{
    ...
    public class Client
    {
        ...
        /// <param name="device">The device that is used to communicate with the Hisser Server.</param>
        public Client(ContactManager context, NetworkCredential credentials, string hostname, Device device)
        {
        ...
        {
            List<MessageHeader> msgs = await _server.GetMessageHeaders(_device);
        ...
        public async Task<SentMessage> SendMessage(Contact receiver, object data, ContentType type) {
        ...
        public async Task<SentInvitation> SendInvitation(string senderAddress, string receiverAddress)
        {
            if (senderAddress != receiverAddress)
            {
                ...
                {
                    throw new Exception(string.Format("You cannot invite a Contact with status {0}", receiver.Status));
                }
            }
            else
            {
        ...
        public async Task<int> CheckMessages()
        {
            ...
            try
            {
                unreadMessages = await _server.GetMessageHeaders(_device);
            }
            catch (Exception ex)
            {
            ...
            if (reportedException is InvalidDeviceIdentifierException)
            {
                ...
                try
                {
                    await _server.RegisterDevice(_device);
                    unreadMessages = await _server.GetMessageHeaders(_device);
                }
                catch (Exception ex)
                {
        ...
        private async Task ProcessInvitation(ReceivedInvitation ir)
        {
            ...
            string senderUsername = Contact.ParseUsername(ir.SenderAddress);
        ...
        /// <summary>
        /// Deletes all unused aliases from the Hisser Server.
        /// </summary>
        /// <returns></returns>
        private async Task DeleteUnusedAliases()
        {
