...
namespace Hisser.ClientServer
{
    ...
    public class MessageHeader
    {
        ...
        /// <returns></returns>
        public static MessageHeader FromStream(HisserStream stream)
        {
            long messageID = stream.ReadInt4();
            long messageSize = stream.ReadInt4();
            MessageType messageType = MessageHeader.ParseMessageType(stream.ReadString1());
        ...
        public static MessageType ParseMessageType(string message_type_string)
        {
            switch (message_type_string)
            {
                ...
                default:
                    {
                        throw new TechnicalException(string.Format("Unsupported message type: {0}", message_type_string));
                    }
            }
        }
        ...
        public static string MessageType_ToString(MessageType message_type)
        {
            switch (message_type)
            {
                ...
                default:
                    {
                        throw new TechnicalException(string.Format("Unsupported message type: {0}", message_type));
                    }
            }
        }
    }
