...
namespace Hisser.ClientServer
{
    ...
    public class SentComponent : SecretComponent
    {
        ...
        /// </summary>
        [Column(DbType = "ntext")]
        public BigInteger PrivateValue { get; private set; }
        ...
        internal SentComponent(long serial)
            : base(serial)
        {
            PrivateValue = new BigInteger(RandomGenerator.GetString(PRIVATE_VALUE_LENGTH, "0123456789"));
            PublicValue =  Secret.g.ModPow(PrivateValue, Secret.p);
        }

        internal SentComponent()
            : base()
        {
        }
    }
